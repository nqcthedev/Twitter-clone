...
//   // Còn nếu tính theo tỉ lệ nhị phân thì 1MB = 2^20 bytes (1024*1024)
...
//   // Thường đấy sẽ là chunkSize, ngoại trừ đoạn cuối cùng
...
export const serveSegmentController = async (req: Request, res: Response, next: NextFunction) => {
  const { id, step, segment } = req.params;
  // return res.sendFile(path.resolve(UPLOAD_VIDEO_DIR, id, step, segment), (err) => {
  //   if (err) {
  //     res.status((err as any).status).send('Not found');
  //   }
  // });
  sendFileFromS3(res, `videos-hls/${id}/${step}/${segment}`);
};
