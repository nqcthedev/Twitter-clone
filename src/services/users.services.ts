...
import { sighToken, verifyToken } from '~/utils/jwt';
...
class UsersService {
  ...
  async resendVerifyEmail(user_id: string, email: string) {
    ...
    // Update value email_verify_token trong document user
  ...
  async forgotPassword({
    ...
    // Send link to user with link : https://twitter.com/forgot-password?token=token
  ...
  async changePassword(user_id: string, new_password: string) {
    await databaseService.users.updateOne({ _id: new ObjectId(user_id) }, [
      { $set: { password: hashPassword(new_password), update_at: '$$NOW' } },
    ]);
    return {
